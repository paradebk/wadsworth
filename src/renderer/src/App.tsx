...
function FileTypeIcon({
  ...
  const dot = name.lastIndexOf('.')
  const ext = dot > 0 ? name.slice(dot + 1) : ''
  if (ext === 'pdf') return <FileType size={14} className="icon-pdf" />
  if (IMAGE_EXTS.has(ext)) return <FileImage size={14} className="icon-image" />
  if (ext === 'csv' || ext === 'tsv' || ext === 'xlsx' || ext === 'xls')
    return <FileSpreadsheet size={14} className="icon-spreadsheet" />
  if (ext === 'md' || ext === 'markdown' || ext === 'txt' || ext === 'rst')
    return <FileText size={14} className="icon-text" />
  if (TEXT_EXTS.has(ext) || TEXT_BASENAMES.has(name))
    return <FileCode size={14} className="icon-code" />
  return <File size={14} className="icon-file" />
}
